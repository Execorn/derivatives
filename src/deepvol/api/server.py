...
def _get_model(model_name: str):
    if model_name not in _MODEL_CACHE:
        ...
        if model_name == "heston":
            ...
            path = Path(__file__).parents[3] / "artifacts" / "weights" / "fno_heston_final_prod.pth"
        elif model_name == "sabr":
            model = MirrorPaddedFNO2d(param_dim=3)
            path = Path(__file__).parents[3] / "artifacts" / "weights" / "fno_sabr_final_prod.pth"
        elif model_name == "ssvi":
            model = MirrorPaddedFNO2d(param_dim=11)
            path = Path(__file__).parents[3] / "artifacts" / "weights" / "fno_ssvi_final_prod.pth"
        elif model_name == "rbergomi":
            model = MirrorPaddedFNO2d(param_dim=4)
            path = Path(__file__).parents[3] / "artifacts" / "weights" / "fno_rbergomi_final_prod.pth"
        else:
