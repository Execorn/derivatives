...
Architecture:
    Input(5) → [Linear(5,30) → ELU] → [Linear(30,30) → ELU] × 3 → Linear(30,88)

The network maps 5 Heston parameters (v0, rho, sigma, theta, kappa) to an
88-point implied volatility surface (8 maturities × 11 strikes).
...
class HestonSurrogateMLP(nn.Module):
    ...
    Maps scaled Heston parameters to a standardized implied volatility surface.
    Uses ELU activations throughout to ensure C² smoothness of the learned map,
    which is critical for downstream gradient-based calibration (Levenberg-Marquardt,
    L-BFGS-B, etc.) that requires well-behaved Jacobians.

    Args:
        input_size:  Number of Heston model parameters (default: 5).
        output_size: Number of IV surface grid points (default: 88 = 8 mat × 11 strikes).
    ...
    Example:
        ...
        >>> iv_surface = model(params)    # shape: (32, 88)
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Tensor of shape (batch_size, output_size) — standardized IV surface.
        """
